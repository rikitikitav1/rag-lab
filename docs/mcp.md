...
- `preregister(name, population, arms, closing, guards?, declared?)` / `preregistration(name)` / `close_preregistration(name, runs)` - what a run promises, written before its first row and never edited: the question sets, control and arm, the closing columns with the direction the arm should move them, and guards with the direction they must not move. Every column is a name from one registry, so it has one reading. Closing computes only what was declared: the paired effect, the floor from a second control pass, each guard as holds, broken, undecided or unreadable, and `cleared` with its reason. A run queued with `purpose: closing` must name a promise that exists.
