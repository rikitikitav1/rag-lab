...
- **Агент на LangGraph**: tool-calling граф, где модель сама решает когда искать по корпусу, может переформулировать запрос и сделать несколько хопов, затем отвечает. Начинался как рукописный цикл; цикл и граф прогнаны друг против друга на одних вопросах и совпали в пределах собственного шума стенда, после чего цикл убран. Политики вокруг цикла (гейт покрытия, ось темы, допуск инструментов) остались нашими и измерены, а не приняты на веру. Выбирается как eval-пайплайн (`pipeline: agent`) и меряется в лоб против single-shot RAG (см. лог экспериментов).
...
- **Bootstrap на старте** (idempotent): завести Model-строки из конфига → засидить роли → сверить с Ollama (что не скачано → статус `loading` + джоба pull) → заэнкьюить индексацию, только если в базе нет ни одного варианта корпуса → заэнкьюить эмбеддинг вопросов без вектора, если такая джоба ещё не стоит. Названный, но пустой вариант пишется в лог, а не индексируется: индексация варианта это осознанный измеряемый шаг.
...
- `service.retrieval` - `distance_threshold`, `results_limit`, `rrf_k`, лимиты кандидатов и тумблеры ключевой ноги: `keyword_query` (`and` требует все лексемы, `or` срабатывает на любую), `keyword_rank`, `keyword_norm`, `query_lang` (`langdetect` или `cyrillic_ratio`). Значения выбраны замером и пишутся в снапшот каждого прогона.
...
### Три реализации одного агента (и четвёртая, которую убрали)

Поток выше начинался самописной петлёй: свой счётчик хопов, свой диспетчер, гейт покрытия вшит между ходами. Теперь его исполняет граф, а параметр `orchestrator` выбирает реализацию тех же политик. Каждая заполняет один и тот же `AgentResult`, поэтому логирование, судья и метрики их не различают.
...
Самой петли больше нет. Она жила ровно до того момента, как перенос был против неё замерен: одни вопросы, одни настройки, совпадение в пределах собственного шума стенда по каждому ключу, который читает пайплайн. Держать вторую реализацию поведения, уже закреплённого тестами, смысла нет, а стоит она ветвлением в каждом будущем изменении. Её прогоны остались в логе под `orchestrator=agent`, цифры в [записи](experiments/2026-08-26_the-same-agent-written-four-ways.md).
...
- `app/orchestrators/` - адаптеры к фреймворку: `graph` (StateGraph), `middleware` (наши политики хуками), `react` (голый `create_agent`). Ни одного импорта langchain в `use_cases`.
