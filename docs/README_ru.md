...
- **Окупается ли фильтрация найденных кусков через LLM?** Грейдер между поиском и ответом сохраняет 0.976 золотых секций и выбрасывает 0.44 чужих кусков, что при том же удержании лучше порога на кросс-энкодере (+0.23 по строкам, интервал не задевает ноль), и всё равно не берёт двусторонней планки, поставленной этим стендом. По умолчанию выключен: четыре секунды на вопрос, 2.4% строк остаются без контекста, и замера, что отфильтрованный контекст даёт более обоснованный ответ, пока нет. [Запись](experiments/2026-09-19_a-grader-that-does-not-buy-it.md)
...
- `datasets/`: наборы вопросов и файлы замеров.
...
Диаграммы это D2-исходники в `docs/diagrams/`, рендер `scripts/render_diagrams.sh`; CI падает, если закоммиченный SVG разошёлся с исходником.
