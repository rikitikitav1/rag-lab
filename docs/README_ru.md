...
| `OLLAMA_CONTEXT_LENGTH` | `12288` | Окно контекста, с которым сервер грузит модели. Дефолт Ollama это 4096, и промпт длиннее он обрезает молча: у агента хопы доходили до 13 тысяч токенов, а 261 прогон из 6198 был длиннее 4096. Замерено на этой 8-гигабайтной карте: при 12288 генератор и эмбеддер живут на карте одновременно (12 загрузок модели, 4 секунды на вопрос), при 14336 начинают выгружать друг друга (112 загрузок, 16 секунд). Рост окна стоит видеопамяти под KV-кэш, поэтому после изменения смотри в логе `llm.model_spilled_to_cpu`. |
