...
- `service.agent` - `max_hops` (кап хопов ReAct), `fallback_policy` (`corpus_first` / `corpus_first_weak` / `agent_choice`), `gate_candidates` и `weak_threshold` (гейт кросс-энкодера над ретривалом).
...
- `corpus_first_weak` - то же плюс гейт на кросс-энкодере: верхние `agent.gate_candidates` результата скорятся, и если лучший ниже `agent.weak_threshold`, ретривал считается промахом (`fallback_reason: weak`). Слабые чанки при этом выбрасываются из диалога, а не идут в ответ, но только когда есть внешний тул, готовый подхватить. Гейту не нужен включённый реранк: при `rerank: false` он скорит пять пар и не трогает порядок, при включённом переиспользует уже посчитанные скоры.
