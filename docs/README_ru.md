...
- `service.agent` - `max_hops` (кап хопов ReAct), `fallback_policy` (`corpus_first` / `corpus_first_weak` / `agent_choice`), `gate_signal` (что считает ретривал слабым: `distance`, `cross_encoder` или `either`) с порогами `weak_distance` и `weak_threshold`, `gate_candidates` (сколько результатов скорит кросс-энкодер), `topic_threshold` (ось темы, по умолчанию выключена).
