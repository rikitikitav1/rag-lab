...
- `config.yaml` и файлы в `config/` (монтируются в контейнер), читаются как один конфиг. `config.yaml` это база: поиск, реранк, агент, нарезка, полнотекстовый поиск, варианты корпуса и источники, движки. Каждый файл в `config/` принадлежит одному процессу: `roles.yaml` роли, их модели и версии промптов, с которых стартует пустая база (`roles.cpu.yaml` подменяет их для стенда без GPU), `intake.yaml` добавление источника (маршрут по файлу, границы отчёта, движок и настройки каждого конвертера), `ingest_quality.yaml` гейты нарезки, `evals.yaml` наборы вердикта и пороги статистики. Раздел, записанный в двух файлах, не загрузится. У значения, выбранного замером, рядом записаны причина и файл замера, и каждый прогон пишет использованные значения в свой снимок.
...
- `sources/<имя>.yaml`: отработанный источник, его происхождение, язык, лицензия и правила, нужные только ему (дерево категорий, пропускаемые файлы, семейства вето); файл главнее строки в базе. `formats/<имя>.yaml`: словарь формата (разделы DocBook, лестница заголовков LaTeX), общий для всех источников этого формата. Новый источник сперва пробуется через `POST /v1/source`.
- код: разбор, который файлом не сказать, и пределы протокола.
