...
- **Прогон говорит, что обещал, до старта.** Закрывающий прогон называет предрегистрацию, запись в базе о своей популяции, плечах, закрывающих колонках и охранных, и очередь отказывает имени, которого там нет. Закрывающее число потом считается из этой записи через ops-сервер MCP, так что колонку, выбранную после взгляда на числа, выдать за результат нельзя. [Инструменты](mcp.md)
