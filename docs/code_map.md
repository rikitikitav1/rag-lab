...
- `app/mcp_ops.py` - ops MCP server (mounted at `/mcp-ops`): `agent_trace` / `broker_balances` / `cancel_job` / `close_preregistration` / `compare_pools` / `compare_runs` / `engines` / `experiment_results` / `holm_over` / `judge_correlation` / `language_cost` / `list_jobs` / `preregister` / `preregistration` / `question_sets` / `questions` / `run_metrics` over the eval platform. A test pins this list to the source, so a new door is named here or the build fails.
