...
- `app/evals/` - eval bench: the runner, retrieval and generation metrics, the guest axes (`guest_axes`, `guest_llm`, `guest_probes`), the judge-against-judge report (`judge_correlation`), the language probe, the replay (`replay`), what a run still owes (`run_debts`), what a question set holds (`question_sets`), the blind pairs the owner ranks (`human_anchor`), what the axes charge for answering in the language asked (`language_cost`), what the agent's own record says about its hops and nodes (`trace`) and where a job leaves its number (`measurements`).
