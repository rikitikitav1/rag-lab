...
- `app/mcp_ops.py` - ops MCP server (mounted at `/mcp-ops`): `run_metrics` / `compare_runs` / `compare_pools` / `judge_correlation` / `question_sets` / `questions` / `experiment_results` / `list_jobs` / `cancel_job` / `holm_over` / `engines` / `language_cost` / `broker_balances` / `agent_trace` over the eval platform.
