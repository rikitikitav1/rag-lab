...
How the GPU moves between engines, in every mode that has one:

<details>
<summary>Diagram: how the GPU moves between engines</summary>

![GPU handover: who asks, the one road, each step and where a failed check leads](diagrams/card_handover.drawio.svg)

</details>

After a handover to a vLLM that serves the generator, the job also asks the woken server whether it returns tool calls; a failed probe is recorded as the probe's, not as a lost GPU.

**Where you are, read in this order, every time:** the ops MCP tool `engines` (who holds the GPU,
...
Two rules hold in every mode. The GPU is handed only through the queue (`hand_card`, or a job taking
...
replaces it in mode 8; the GPU memory share of each vLLM service is its own variable (`VLLM_GPU_UTIL`,
...
With the embedder and the generator on two GPU engines, the chat answers 409: it does not
hand the GPU between them within one answer. A run does, one handover per call of the other role.
...
kernels than the GPU's, so they are a different instrument, not a slower copy of the same one.

Leave it: seat the roles back on their GPU engines, then `docker compose stop vllm-cpu`.
...
on ollama, so the chat answers 409 as in mode 3 and a run hands the GPU over per call.
...
answering it; a job ends with an error about the GPU handover.

```bash
curl -s "localhost:8000/v1/job?type=hand_card&status=error&limit=5" | python3 -m json.tool
```

- `... did not let go of the card in 60s`: an engine kept a model on the GPU; `engines` names it.
- `... did not wake in 60s`: the vLLM could not take the GPU back, usually because something else
...
- `... is down; the card stays where it is`, `... is unknown; ...`: the target server is stopped or
  ...
  restarted with `docker compose restart vllm`, and it comes back awake on the GPU.
- `... loaded on ollama, but not whole on the card`: the model landed half on the processor; the
  GPU is short of memory, and `engines` with `nvidia-smi` say who holds it.
- `nvidia-smi` inside a container answers `Failed to initialize NVML: Unknown Error`, and jobs end
  with `not whole on the card`, which `/readiness` names by role: the container lost the GPU. With the GPU given through CDI a
  `systemctl daemon-reload` no longer does this; if it happens anyway, `docker compose restart
  <service>` gives the GPU back, and `docker info` should list the NVIDIA CDI devices (README,
  Quickstart).
...
- `... did not let go of its tool; the restart stops here`: a converter's piece hung past its bound, the
  job asked the supervisor to stop the tool and take it again, and the child would not die, so it still
  holds the GPU. `docker compose restart converter-docling` (or `converter-mineru`) kills it; the next
...
not the GPU's ollama, which this mode leaves out. On a host with a GPU it names a role whose GPU
engine is down and points here.

Checked on Linux with the GPU not given to the containers. A Mac is expected to behave the same and
...
![A cloud call: how it is asked, what an answer passes through, and where each refusal leads](diagrams/cloud_call.drawio.svg)

</details>

A cloud role never takes the GPU, and a call answers in seconds. What differs from a local engine:
