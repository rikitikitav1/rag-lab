...
- **What a role requires of a model** (tool calling for the agent's generator, no reasoning-only judge, the card arithmetic, and why the role's name is served from the database rather than the file): [model_requirements.md](model_requirements.md).
