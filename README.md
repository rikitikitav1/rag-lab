...
- **Does filtering the retrieved chunks with an LLM pay off?** A grader between search and the answer keeps 0.976 of the gold sections and drops 0.44 of the strangers, which beats a threshold on the cross-encoder at the same retention (+0.23 by row, interval clear of zero) and still misses the two-sided bar this bench set for it. It is off by default: four seconds a question, 2.4% of rows left with no context, and no measurement yet that a filtered context is better grounded. [Entry](docs/experiments/2026-09-19_a-grader-that-does-not-buy-it.md)
