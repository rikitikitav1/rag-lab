...
A single run does not loop per question: it goes through phases so each stage owns the GPU alone, which is what makes reranking affordable in bulk. Per question the loop needed the embedder, then the reranker, then the generator, and the three do not fit in 8 GB together, so ollama evicted and reloaded a model on every single question. Phases cost a few model swaps per run instead of two per question, and the run gives the card back when it ends, so a queue of runs on different generators does not end up holding two of them at once. The reranker finally gets a real batch: 44 s for 100 questions on the card against about 16 min on CPU. That 440 ms a question is the whole phase, model load and retrieval included; the reranking itself is the 86 ms measured in `datasets/measurements/rerank_latency.json`.
...
| `prefix_1024` | `structured` | the same again plus one more level: a section is cut by its subheadings before it is cut by size. Measured and dropped: no gain shown, and a variant nobody serves still pays into every other variant's query plan |
...
`baseline → clean_1024` on 823 questions is **+0.0435 section MRR** with all six intervals clear of zero, reported on the pre-registered half. `clean_1024 → prefix_1024` on the same questions is **−0.0109 [−0.0253, +0.0032]**: no winner on the half where the winner is chosen, so the third heading level bought nothing and was removed.

### How deep the index is walked, and why that is not a number

`ef_search` defaults to `auto`, which means the deepest rung of `ef_ladder` whose plan still contains an `Index Scan`, asked of the planner rather than remembered. Where the planner abandons the walk is a property of the whole table: the alternative is a sequential read of `data_chunks` filtered by variant, priced per page, so rows of every other variant pay into it. Indexing one more variant moved that point from about 197 to about 265; deleting the variant again moved it back not at all, because a `DELETE` leaves the pages where they were.

The answer is cached against the two statistics the planner itself reads (`relpages`, `reltuples`) and re-asked when either moves. It is re-read at the end of every index build, and the preflight audits every indexed variant rather than the served one alone. A number in `config.yaml` pins it instead, and so does `ef_search` on a request, on a run, or as a comparison axis; what a run used is a number in its snapshot, never the word `auto`.
