...
- `llm.roles` - model, optional `engine` and `options` per role (`generation` / `embedding` / `judging` / `paraphrasing`); the name here is what bootstrap assigns on an empty database, and the database serves it afterwards. A role that names an engine is seated on that engine or nowhere, and its model is registered through `/v1/model` rather than pulled; a role that names none takes the seeded ollama. `llm.candidates` - models to pull but not assign. `llm.context_length` - what we expect `OLLAMA_CONTEXT_LENGTH` to be, which the preflight compares against the server. `llm.base_url` - where ollama listens. The judge's model on vLLM is named here and nowhere else: `scripts/up.sh` passes it to the `vllm` service.
