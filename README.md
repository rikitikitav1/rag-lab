...
| `OLLAMA_CONTEXT_LENGTH` | `12288` | Context window the server loads models with. Ollama defaults to 4096 and truncates a longer prompt without a word: agent hops reached 13k tokens here, and 261 of 6198 logged runs were over 4096. Measured on this 8 GB card: at 12288 both the generator and the embedder stay resident (12 model loads, 4s per question), at 14336 they start evicting each other (112 loads, 16s). Raising it costs VRAM for the KV cache, so check `llm.model_spilled_to_cpu` in the log after a change. |
