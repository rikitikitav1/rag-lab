...
- **Models by role.** There are seven roles: five of our own (generation, embeddings, judging, paraphrasing, reranking) and two for the RAGAS guest judge, its model and its embedder. Which model serves a role is stored in the database and can be changed without a restart. Each role sits on an engine: ollama or vLLM, on the card or on the CPU, or a cloud API.
