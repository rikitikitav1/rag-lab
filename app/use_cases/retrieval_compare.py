...
# 1 was the shape before the halves and the per-arm procedure; a reader tells the two
# apart by this rather than by the date the row was written
SCHEMA = 2
...
# the gold and the section are compared through these two, so a set generated for this
# comparison has to be generated by them: exported rather than private
def clean_gold(text):
...
# two variants can hold different files, not only different cuts of the same ones, and a
# delta over a family whose file sets differ is part cut and part corpus. Only the
# families that differ come back; for the rest there is nothing to say
FILE_DRIFT_SQL = """
WITH files AS (
