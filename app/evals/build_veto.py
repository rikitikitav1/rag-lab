...
def candidates(variants: list[str], cut_from: str) -> list[dict]:
    with Session() as session:
        ...
        taken = set(
            session.scalars(
                select(Question.text_hash).where((Question.set_name != ORIGINALS) | Question.set_name.is_(None))
            )
        )
    ...
    for source, section, language in rows:
        ...
        seen.setdefault(key, []).append({"family": family, "source": source, "heading": leaf, "language": language})
    picked = []
...
def _order_key(row: dict, seed: str) -> str:
    return hashlib.md5(f"{row['source']}\x00{row['heading']}\x00{seed}".encode(), usedforsecurity=False).hexdigest()
...
def plan(seed: str, variants: list[str], cut_from: str, quotas: dict | None = None) -> list[dict]:
    ...
    for family, quota in sorted(quotas.items()):
        of_family = sorted((r for r in rows if r["family"] == family), key=lambda r: _order_key(r, seed))
        if len(of_family) < quota:
            log.warning("veto.short_of_quota", family=family, asked=quota, have=len(of_family))
        out += of_family[:quota]
    return out
...
def _add_question(session, values: dict) -> int | None:
    return session.scalar(
        pg_insert(Question).values(values).on_conflict_do_nothing(index_elements=["text_hash"]).returning(Question.id)
    )
...
def _originals(session, rows: list[dict]) -> dict[str, int]:
    ...
    for row in rows:
        ...
        ids[digest] = made or session.scalar(select(Question.id).where(Question.text_hash == digest))
    session.commit()
    return ids
