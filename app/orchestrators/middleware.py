...
class CoverageGateMiddleware(AgentMiddleware):
    ...
    def before_model(self, state, runtime):
        # the loop asks one last time without tools, and its prompt belongs in the transcript
        if (
            self.run.model_calls >= self.run.max_hops
            and not self.run.sources
            and not self.run.no_evidence_prompted
        ):
        ...
        # the loop collects sources in call order, and the two arms are compared row by row
...
class ToolboxMiddleware(AgentMiddleware):
    ...
    def wrap_model_call(self, request, handler):
        ...
        # the loop answers one last time without tools when the hops are spent or a turn came back
        # empty, and that last answer is the one the run is judged on
...
# a turn that came back with nothing gets the same last answer the loop asks for
...
def build(run: Run) -> list:
    ...
    # Nudge has to see a turn before EmptyTurn does, hence its place at the end of the list
    return [
