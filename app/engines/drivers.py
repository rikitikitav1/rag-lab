...
class Vllm(Driver):
    ...
    def refuse_delete(self, name):
        for spec in registered():
            if spec.kind is not EngineKind.vllm:
                continue
            try:
                names = vllm.served(spec)
            # first: a ConnectTimeout is a ConnectionError too, and a host that did not answer may serve it
